...
EM_biprobit_partial = function(par,y1,y2,x1,x2,maxIter=200,tol=1e-6,tol_LL=1e-8,verbose=1,accu=1e4){
    par_last = par
    LL_last = -Inf
    for(i in 1:maxIter){
        if(verbose>=1) {
        ...
        # 2. M step
        est = optim(par=par, fn=Q_biprobit_partial, gr=NULL, method="BFGS", control=list(factr=accu,fnscale=-1), P1=P1, y1=y1, y2=y2, x1=x1, x2=x2, verbose=0)
        par = est$par
...
#' @description Estimate two probit models with bivariate normally distributed error terms, in which the dependent variable of the first stage model is partially observed (or unobserved)
...
biprobit_partial = function(form1, form2, data=NULL, EM=FALSE, par=NULL, method='BFGS', verbose=0, accu=1e4, maxIter=500, tol=1e-5, tol_LL=1e-6){
    # 1.1 parse y1~x1
    ...
    if(EM==TRUE){
        res = EM_biprobit_partial(par,y1,y2,x1,x2,maxIter=maxIter,tol=tol,tol_LL=tol_LL,verbose=verbose,accu=accu)
    } else {
        ...
        # res = optim(par=par, fn=LL_biprobit_partial, gr=NULL, method=method, control=list(factr=accu,fnscale=-1), y1=y1, y2=y2, x1=x1, x2=x2, verbose=verbose, hessian = TRUE)
