LL_probit_linear_partial = function(par,y1,y2,x1,x2,verbose=1){
    ...
    z1 = (y2-w2-gamma)/sigma
    z0 = (y2-w2)/sigma

    # errors can be amplified due to multiplication?
    Li = (1-disable1) * dnorm(y2-w2-gamma, sd=sigma) * pnorm( (w1+z1*rho)/sqrt(1-rho^2) ) + (1-disable0) * dnorm(y2-w2, sd=sigma) * pnorm( -(w1+z0*rho)/sqrt(1-rho^2) )
...
Q_probit_linear_partial = function(par,P1,y1,y2,x1,x2,verbose=1){
    if(length(par) != ncol(x1)+ncol(x2)+3) stop("Number of parameters incorrect")
    alpha = par[1:ncol(x1)]
    beta = par[ncol(x1)+1:ncol(x2)]
    gamma = par[length(par)-2]
    ...
    Li = P1 * ( dnorm(y2-w2-gamma, sd=sigma, log=TRUE) + pnorm( (w1+z1*rho)/sqrt(1-rho^2), log.p=TRUE) ) + (1-P1) * ( dnorm(y2-w2, sd=sigma, log=TRUE) + pnorm( -(w1+z0*rho)/sqrt(1-rho^2), log.p=TRUE) )
...
EM_probit_linear_partial = function(par,y1,y2,x1,x2,maxIter=200,tol=1e-8,tol_LL=1e-8,verbose=1,accu=1e4){
    ...
    for(i in 1:maxIter){
        ...
        L1 = dnorm(y2-w2-gamma, sd=sigma) * pnorm( (w1+z1*rho)/sqrt(1-rho^2) )
        L0 =  dnorm(y2-w2, sd=sigma) * pnorm( -(w1+z0*rho)/sqrt(1-rho^2) )
        P1 = L1 / (L1+L0)
    ...
    est$hessian = numericHessian(LL_probit_linear_partial, t0=par, y1=y1, y2=y2, x1=x1, x2=x2, verbose=0)
    # est$hessian2 = numericHessian(Q_probit_linear_partial, t0=par, P1=P1, y1=y1, y2=y2, x1=x1, x2=x2, verbose=0)
    est$gradient = numericGradient(LL_probit_linear_partial, t0=par, y1=y1, y2=y2, x1=x1, x2=x2, verbose=0)
    # est$g2 = numericGradient(Q_probit_linear_partial, t0=par, P1=P1, y1=y1, y2=y2, x1=x1, x2=x2, verbose=0)
...
probit_linear_partial = function(form_probit, form_linear, data=NULL, EM=TRUE, par=NULL, method='BFGS', verbose=0, accu=1e4, maxIter=500, tol=1e-6, tol_LL=1e-8){
    ...
    names(par) = c(paste0('probit.', colnames(x1)), paste0('linear.', colnames(x2)), 'gamma', 'log_sigma', 'tau')
    # print(par)
    # 2. Estimation
    resetIter()
    begin = Sys.time()
    ...
    if(EM==TRUE){
        ...
        res = maxLik(LL_probit_linear_partial, start=par, method=method, y1=y1, y2=y2, x1=x1, x2=x2, verbose=verbose)
